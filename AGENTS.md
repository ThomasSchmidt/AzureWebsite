...
- Run all tests and collect code coverage with `dotnet test --collect:"XPlat Code Coverage" -- DataCollectionRunSettings.DataCollectors.DataCollector.Configuration.ExcludeByAttribute="GeneratedCodeAttribute"`, the last part is a setting that tells it to ignore autogenerated code which we shouldn't test.
